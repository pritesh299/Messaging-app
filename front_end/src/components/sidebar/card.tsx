...
const Card: React.FC<CardProps> = ({
  ...
  const [userExits, setUserExits] = useState<boolean>(true);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const isoString = lastMessage ? lastMessage.updatedAt : "";
      ...
      if (lastMessage.senderId !==getGlobal("id") && !lastMessage.isRead) {
      ...
      setUser({
        ...
        lastMessageSeen: lastMessage.isDelivered,
    ...
    <>
      {userExits && (
        <button
          ...
          className={`card h-[70px] gap-[5px] text-white flex justify-between items-center w-full ${
            hover ? "bg-[#273443]" : "bg-[#111b21]"
          }`}
        >
          {/* Avatar */}
          ...
          <div className="w-[80%] border-b py-3 border-slate-700 flex">
            ...
            <div className="text-left w-[80%]">
              ...
              <div className="flex gap-2 items-center">
                {/* Double tick */}
                {user?.lastMessageSenderId === getGlobal("id") && (
                  <svg
                    viewBox="0 0 16 11"
                    height="11"
                    width="16"
                  ...
                  >
                    ...
                    <path
                      ...
                      fill={user?.lastMessageSeen ? "#3b82f6" : "currentColor"}
                    />
                  </svg>
                )}
                <p className="text-slate-400 text-sm">
                  {reduceMessage(user?.lastMessage)}
                </p>
              </div>
            </div>
            ...
            <div className="w-[20%] flex flex-col items-center justify-between">
              ...
              {unReadMessages && (
                <div className="mt-1 h-[10px] w-[10px] bg-green-500 rounded-full"></div>
              )}
            </div>
          </div>
        </button>
